...
- `harness instance reset --from-local` materialises a blueprint locally from `workshop-content/agenda.json` and sends it to the reset API — useful when content has changed locally but the dashboard has not been redeployed. Language/mode default to `cs` / `facilitator`; override with `--content-lang en` or `--agenda-mode participant`.
