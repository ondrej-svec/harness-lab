...
| `HARNESS_EVENT_CODE_REVEAL_KEY` | optional (file mode disables reveal gracefully) | **required** (base64url, 32 bytes) | **required** (base64url, 32 bytes) | AES-256-GCM key for facilitator-side event-code reveal. Separate from the HMAC secret so rotation is independent. Losing the key means losing reveal for codes issued under it — the hash still redeems | `lib/event-code-reveal-crypto.ts` |
