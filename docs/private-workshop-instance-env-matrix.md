...
| `HARNESS_WORKSHOP_INSTANCE_ID` | optional, defaults to seed instance | optional, defaults to first DB instance | optional, defaults to first DB instance | Default instance for public page and facilitator fallback. Participant sessions resolve their own instance from the DB — this variable does not gate participant access. |
...
- preview `HARNESS_WORKSHOP_INSTANCE_ID` is optional; when set, it determines the public homepage default instance
...
- `HARNESS_WORKSHOP_INSTANCE_ID` is optional in production; when set, it determines the public homepage default. Participant event codes resolve to their own instance automatically.
