...
- [x] Facilitator-side walk-in toggle + per-row "reset password" action in the People section. Walk-in toggle: server action `toggleWalkInsAction` in `_actions/access.ts` mirrors the API endpoint, surfaces in `AccessSection` as a small radio + save form using the same audit shape (`facilitator_walk_in_policy`). Reset password: client-side button on each pool row when `participant.neonUserId !== null`, POSTs to the existing API endpoint, surfaces the returned 3-word temp password in a dismissible banner at the top of the People section. Czech + English copy in `ui-language.ts` (`walkInPolicy*`, `participantResetPassword*`).
