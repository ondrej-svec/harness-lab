...
- [x] **A4. Regression test for installer split.** Add a test (match the existing harness-cli test shape) that asserts (a) default `installWorkshopSkill({ cwd })` produces no files under `.claude/skills/workshop-facilitator` or `.agents/skills/workshop-facilitator`; (b) `installWorkshopSkill({ cwd, facilitator: true })` produces both. Runs under whatever test runner `harness-cli` already uses. *Depends on: A1.*
...
**P0**
- [ ] From a fresh clone with `harness skill install` (no flags), no file exists under `<target>/.claude/skills/workshop-facilitator/` or `<target>/.agents/skills/workshop-facilitator/`.
