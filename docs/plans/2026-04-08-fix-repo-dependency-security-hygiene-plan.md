...
| `harness-cli` should stay on a single pinned major | Rejected later | Participant installs on newer local Node majors produced avoidable `EBADENGINE` friction; the CLI support surface is better modeled as a verified minimum floor |
...
- [x] Replace floating engine ranges with intentional supported Node policy in both package roots, based on Phase 1 decisions.
...
- [x] `dashboard` and `harness-cli` declare intentional supported Node policy rather than floating `>=24` ranges.
