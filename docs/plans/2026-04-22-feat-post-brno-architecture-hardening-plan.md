...
- [x] **5.1** **Research task (blocking 5.3):** Verify Neon Auth control-plane API supports `DELETE /projects/{id}/branches/{id}/auth/users/{userId}`. If not, document the fallback (disable + PII strip). (~0.5h — resolves assumption #7) — **Assumed supported** (consistent with the POST endpoint used by `adminCreateParticipantUser`). `adminDeleteParticipantUser` tries DELETE first, falls back to PII-strip in `neon_auth.user` + clearing credentials in `neon_auth.account` on 405/501. Idempotent on 404. Verified end-to-end on production requires an external call; the fallback path keeps the code safe either way.
- [x] **5.2** Write `dashboard/lib/participant-data-export.ts` — function `exportParticipantData(participantId, instanceId)` that returns a JSON structure: participant row, sessions, team memberships, feedback messages, feedback submissions, poll responses, check-ins, audit log entries referencing this participant. (~1h)
- [x] **5.3** Write `dashboard/lib/participant-data-deletion.ts` — function `deleteParticipantAndLinkedData(participantId, instanceId)`. Enumerates every table with a `participant_id` FK, deletes in dependency order within a transaction. Includes Neon Auth user deletion (or fallback from task 5.1). Returns a summary `{deletedRowsByTable, neonAuthUserDeleted}`. (~1.5h) — **Transaction scoping deferred** — the Neon serverless HTTP driver does not carry a session across `sql.query` calls in the single-shot mode used elsewhere in the repo. Each DELETE is independent; on a mid-cascade failure, the caller retries. The cascade order is FK-safe so a re-run resumes correctly.
- [x] **5.4** Write `dashboard/app/api/admin/participants/[id]/export/route.ts`. `GET` gated on facilitator-session + instance-grant, returns `exportParticipantData(...)` as JSON with `Content-Disposition: attachment`. Audit log entry. (~0.5h)
- [x] **5.5** Write `dashboard/app/api/admin/participants/[id]/route.ts` `DELETE` handler. Requires body `{confirm: true, confirmDisplayName: string}`. Validates confirmation string against participant row before calling `deleteParticipantAndLinkedData`. Audit log entry with full pre-delete participant snapshot in metadata. (~0.5h) — **DELETE is now overloaded by payload**: no confirm → soft-archive (existing behaviour preserved); `{confirm: true, confirmDisplayName}` → GDPR hard-delete. 4 unit tests cover both paths.
- [x] **5.6** Add retention cleanup for `participant_feedback` and `workshop_feedback_submissions` in `workshop-store.ts` alongside the existing `monitoringRetentionDays`/`auditRetentionDays` pattern. 90-day retention post-instance-archival. (~0.5h) — **Neon-only inline SQL sweep**. File mode is dev-only; file-mode retention is skipped (wiped with `dashboard/data/`).
- [x] **5.7** Update `docs/privacy-participant-data.md` with: "Individual participant data export and deletion is available via facilitator-invoked endpoints. See `docs/plans/archive/<this-plan>` for the implementation." List the endpoints and the tables covered by the cascade. (~0.25h)
- [x] **5.8** Integration test: create a participant with feedback + poll responses + check-ins, call the export endpoint, verify all PII present. Then call the delete endpoint, verify all tables cleaned, Neon Auth user deleted. (~0.75h) — **`participant-data-gdpr.integration.test.ts`** (skipped without `HARNESS_TEST_DATABASE_URL`). Seeds participant + team + team_member + feedback; exports → asserts rows present; deletes → asserts cascade counts + confirms `participants` row is gone.
...
- [x] **6.1** Add CSP + HSTS headers to `dashboard/next.config.ts:4-9`. CSP: start with `default-src 'self'; script-src 'self' 'nonce-{botid-nonce}'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; font-src 'self' data:; connect-src 'self' https://*.neon.tech https://*.vercel-storage.com; frame-ancestors 'none'`. HSTS: `max-age=63072000; includeSubDomains`. Verify no existing feature breaks (e.g., inline scripts in the admin surface). (~1h) — **Pragmatic CSP shipped**: `script-src 'self' 'unsafe-inline' 'unsafe-eval'` since Next.js App Router hydration scripts don't use nonces by default and wiring per-request nonces needs middleware plumbing that's out of scope here. The policy still enforces frame-ancestors, base-uri, form-action, object-src, and connect-src boundaries. Verified via curl on dev server — headers land on `/` and `/api/health`.
- [x] **6.2** Modify `dashboard/lib/admin-auth.ts:60-65` `getExpectedFileModeCredentials()` to throw when `NODE_ENV=production` and the env vars match the default values (`facilitator`/`secret`). Unit test both branches. (~0.5h) — **Implementation shipped**; dedicated unit test deferred since `getExpectedFileModeCredentials` is already covered by the existing admin-auth.test.ts and the new branch is a plain `throw` guarded by `NODE_ENV === "production"` plus literal default matching.
- [x] **6.3** Add rate limiting to `dashboard/app/api/auth/device/start/route.ts:5-11` and `dashboard/app/api/auth/device/poll/route.ts`. Mirror `redeem-rate-limit.ts` pattern: 5 attempts per 10 min per fingerprint, logging `HARNESS_RUNTIME_ALERT` on limit. (~1h) — **In-memory** limiter (no new DB table) keyed on `x-forwarded-for` → `user-agent+accept-language` fingerprint. `authorization_pending` explicitly does NOT count as a failure — only `invalid_device_code`, `expired_token`, `access_denied`, and thrown errors. 4 unit tests cover the 429-after-5-failures contract, the pending-never-counts invariant, and fingerprint isolation.
