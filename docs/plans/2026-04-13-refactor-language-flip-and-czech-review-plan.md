...
- [x] **E1. Move CS files to `locales/cs/`.** 12 pairs flipped via `git mv` (preserves history). All talks, briefs, materials kit, master-guide, codex-setup-verification, challenge-cards/deck, print-spec. For each pair listed in the inventory (talks, kit, 5 briefs, plus the newly-created coaching-codex CS from B6): `git mv` the CS file from current location to `locales/cs/`. Use `git mv` (not delete + create) to preserve git history.
- [x] **E2. Promote EN files to root.** Combined with E1 in one pass — for each pair, CS moved to `locales/cs/` then EN moved from `locales/en/` to root. All `locales/en/` directories for in-scope files are now gone. For each pair, `git mv` the EN file from `locales/en/` to the root location.
- [x] **E3. Move newly-created EN files to root.** Covered by E1/E2 — B1 master-guide, B2 codex-setup-verification, B4 print-spec EN versions all promoted from `locales/en/` to root in the same pass. For master-guide, codex-setup-verification, deck, print-spec, codex-demo-script: the EN versions created in B1–B5 (currently in `locales/en/`) move to root, displacing the existing CS files which move to `locales/cs/`.
- [x] **E4. Update path references from A2.** `scripts/content/check-tier2-sync.ts` inverted — `SINGLE_LANGUAGE_ROOT` set replaces `CZECH_ONLY` (removed stale `materials/coaching-codex.md` and `content/challenge-cards/print-spec.md` entries), `collectCzechFiles` walks `locales/cs/`, `englishPathToCzechPath`/`czechPathToEnglishPath` converted. `scripts/content/generate-briefs-inventory.ts` EN_BRIEFS_DIR/CS_BRIEFS_DIR swapped. Tier2 sync check: PASS, 13 synced pairs. Apply every edit needed in scripts, generators, skill, dashboard, content cross-references, frontmatter `sourceRefs`, etc. Test that `npm run generate:content` still produces the same agenda views.
- [x] **E5. Verify the flip didn't break agenda.json.** `sourceRefs` point at paths like `content/talks/context-is-king.md` which still exist (now as EN canonical). `generate:content` regenerated without errors. `dashboard/lib/generated/agenda-{en,cs}.json` and `workshop-blueprint/agenda.json` regenerated cleanly. agenda.json has `sourceRefs` pointing at `content/talks/context-is-king.md` and similar. Update those references to keep pointing at the EN canonical now living at the same root paths (so the references should mostly still work — they were always pointing at the root, which is now EN).
- [x] **E6. Run `generate:content`.** `bun generate-briefs-inventory.ts` + `bun generate-views.ts` both exit clean. Briefs inventory rewrote agenda.json from the new EN root. Views regenerated with the updated cs_reviewed flags. Confirm regeneration produces the same output. If it doesn't, debug Phase E before proceeding.
- [x] **E7. Pause and review the new structure.** Directory walk confirms root = EN, `locales/cs/` = CS for all 12 flipped pairs. tier2-sync passes. generate:content passes. copy-editor verify shows 13 files with no lockfile entry + 499 staleness findings — exactly as expected; F1 re-segmentation is the remedy. A directory walk, a quick smoke test of dashboard rendering, a skill invocation if possible. **Phase F does not start until this passes.**
