...
Use runtime dashboard data for anything that changes during the workshop day or varies per instance:
- current agenda phase
- continuation reveal state
- live team registry
- checkpoints and sprint updates
- project briefs (instance-specific; may differ from the default set)
- challenge cards (instance-specific; may differ from the default set)
- facilitator-only status when the command explicitly requires facilitator access

Use repo-native content as offline fallback only:
- `content/` workshop materials — used when the CLI/API is unreachable
...
Prefer `harness --json workshop brief` for live instance-specific data (via CLI → dashboard API).
If the CLI is unavailable or returns an error, fall back to `content/project-briefs/locales/<locale>/<brief>.md` when present and otherwise `content/project-briefs/<brief>.md`, and say explicitly that the answer is fallback content.
...
Prefer `harness --json workshop challenges` for live instance-specific data (via CLI → dashboard API).
If the CLI is unavailable or returns an error, fall back to `content/challenge-cards/locales/<locale>/deck.md` when present and otherwise `content/challenge-cards/deck.md`, and say explicitly that the answer is fallback content.
