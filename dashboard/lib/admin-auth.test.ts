...
import { decodeBasicAuthHeader, hasValidAdminCredentials, isProtectedPath } from "./admin-auth";
...
describe("admin-auth", () => {
  ...
  it("accepts requests when admin auth is disabled", () => {
    expect(
  ...
  it("validates configured credentials", () => {
    ...
    expect(
      hasValidAdminCredentials({
        authorizationHeader: encodeBasicAuth("someone:secret"),
        configuredPassword: "secret",
      }),
    ).toBe(false);
