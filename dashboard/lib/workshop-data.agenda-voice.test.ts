...
/**
 * Keys holding structural identifiers or non-rendered metadata rather
 * than participant-facing copy. Excluded from the voice-rule scan so
 * scene / block / phase IDs like `opening-team-formation-room` do not
 * count as violations.
 */
...
function collectStrings(
  ...
  if (parentKey !== undefined && STRUCTURAL_KEYS.has(parentKey)) return;
  if (typeof value === "string") {
    out.push({ path, text: value });
    return;
  }
  if (Array.isArray(value)) {
    value.forEach((item, index) => collectStrings(item, `${path}[${index}]`, out, parentKey));
    return;
  }
  if (typeof value === "object") {
    for (const [key, child] of Object.entries(value as Record<string, unknown>)) {
      collectStrings(child, path ? `${path}.${key}` : key, out, key);
    }
  }
}
